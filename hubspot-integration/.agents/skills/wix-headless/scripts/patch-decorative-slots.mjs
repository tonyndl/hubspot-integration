...
if (!imageUrlsResolved.path) {
  console.log(
    JSON.stringify({
      status: "skipped",
      reason:
        "no .wix/image-urls.md (Image Phase 1 didn't write — failed or timed out)",
...
if (Object.keys(urlMap).length === 0) {
  console.log(
    JSON.stringify({
      status: "skipped",
      reason:
        ".wix/image-urls.md exists but has no parseable `## key` + `- url:` blocks",
    }),
  );
  process.exit(0);
}
    ...
    (match, openTag, slotKey, inner, closeTag) => {
      ...
      if (!url) {
        skipped.push({
          file,
          slot: slotKey,
          reason: "no URL for this slot in image-urls.md",
        });
        return match;
      }
