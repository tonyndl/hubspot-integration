...
async function copyTemplate(sourceRel, destPath) {
  if (SKILL_ROOT_DISK) {
    ...
    if (!existsSync(src))
      return { ok: false, reason: `missing on disk: ${src}` };
    try {
...
const TEMPLATES = [
  {
    pack: "stores",
    phase: "components",
    source: "stores/back-in-stock.ts",
    dest: "src/utils/back-in-stock.ts",
  },
  {
    pack: "ecom",
    phase: "components",
    source: "ecom/discounts.ts",
    dest: "src/utils/discounts.ts",
  },
  {
    pack: "stores",
    phase: "pages",
    source: "stores/categories.ts",
    dest: "src/utils/categories.ts",
  },
];
...
if (phase !== "components" && phase !== "pages") {
  console.error(
    `copy-utility-templates: invalid phase "${phase}" — must be "components" or "pages"`,
  );
  process.exit(2);
}
...
try {
  ...
  if (loadedPacks.size === 0) {
    console.error(
      `copy-utility-templates: site.json.verticals is empty — has Setup completed?`,
    );
    process.exit(1);
  }
} catch (e) {
  console.error(
    `copy-utility-templates: cannot read ${sitePath} (${e.message}). Has Setup completed?`,
  );
  process.exit(1);
}

const candidates = TEMPLATES.filter(
  (t) => t.phase === phase && loadedPacks.has(t.pack),
);

if (candidates.length === 0) {
  console.log(
    `copy-utility-templates: no templates for phase "${phase}" with loaded packs ${[...loadedPacks].join(", ")} — nothing to do`,
  );
  process.exit(0);
}
...
for (const { pack, source, dest } of candidates) {
  ...
  if (result.ok) {
    ...
    errors.push(
      `failed to copy ${source} → ${dest} (pack "${pack}"): ${result.reason}`,
    );
  }
}
