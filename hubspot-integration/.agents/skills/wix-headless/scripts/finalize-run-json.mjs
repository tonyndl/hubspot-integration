...
const requiredPhases = Array.isArray(draft.requiredPhases)
  ? draft.requiredPhases
  : [];
...
for (const required of requiredPhases) {
  ...
  if (!existing) {
    ...
    if (
      !errors.some((e) => e.code === "MISSING_TIMING" && e.phase === required)
    ) {
      errors.push({ code: "MISSING_TIMING", phase: required });
    }
    missingTiming.push(required);
...
const buildPlusRelease =
  build !== null && release !== null ? build + release : null;
