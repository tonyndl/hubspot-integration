...
if (!process.stdin.isTTY) {
  ...
  if (raw) {
    try {
      ...
      console.error(
        `init-site-json: stdin contained non-JSON intent payload: ${err.message}`,
      );
      process.exit(2);
    }
    if (
      typeof stdinPayload !== "object" ||
      Array.isArray(stdinPayload) ||
      stdinPayload === null
    ) {
...
if (stdinPayload) {
  if (stdinPayload.imagery !== undefined) {
    if (!VALID_IMAGERY.has(stdinPayload.imagery)) {
      console.error(
        `init-site-json: intent.imagery must be "themed-blocks" or "ai-generated"; got ${JSON.stringify(stdinPayload.imagery)}`,
      );
      process.exit(2);
    }
  ...
  for (const [key, value] of Object.entries(stdinPayload)) {
    ...
    if (!KNOWN_VERTICAL_INTENT_KEYS.has(key)) {
      console.error(
        `init-site-json: unknown intent key "${key}" — expected one of ${[...KNOWN_VERTICAL_INTENT_KEYS].join(", ")}, or "imagery"`,
      );
      process.exit(2);
    }
...
console.log(
  JSON.stringify({ status: "ok", path: sitePath, verticals, intent }, null, 2),
);
