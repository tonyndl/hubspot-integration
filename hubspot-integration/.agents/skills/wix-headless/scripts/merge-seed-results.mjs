...
if (!existsSync(sitePath)) {
  console.error(
    `merge-seed-results: ${sitePath} does not exist — run init-site-json.mjs first`,
  );
  process.exit(2);
}
...
if (!existsSync(returnsDir)) {
  console.error(
    `merge-seed-results: ${returnsDir} does not exist — no subagent returns to merge`,
  );
  process.exit(2);
}
...
for (const file of returnFiles) {
  ...
  if (!expectedPhases.includes(payload.phase)) {
    ...
    errors.push(
      `${pack}: expected phase ${label}, got ${JSON.stringify(payload.phase)}`,
    );
    continue;
  }
  ...
  if (!VALID_STATUS.has(payload.status)) {
    errors.push(
      `${pack}: status must be one of ${[...VALID_STATUS].join(", ")}; got ${JSON.stringify(payload.status)}`,
    );
    continue;
  }
...
console.log(
  JSON.stringify(
    { status: errors.length === 0 ? "ok" : "error", merged: summary, errors },
    null,
    2,
  ),
);
