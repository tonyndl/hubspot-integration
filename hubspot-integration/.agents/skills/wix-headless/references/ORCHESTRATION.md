...
1. **One Phase 3 Components subagent per loaded pack** that declares `components`. Each prompt carries:
   ...
   - A note: _"`src/styles/components-<pack>.css` is already on disk (copied from the skill template by the orchestrator). Do NOT write that file — write only `.tsx` islands and any `.astro` shells in your scope."_
