...
>
> Launch **every** subagent dispatch below in a single concurrent batch (one assistant message containing N `Agent` tool_uses as siblings). That includes per-pack seeders and Image Phase 1 Decorative. No narration, no _"Dispatching seeders:"_, no transition text between dispatches.
>
